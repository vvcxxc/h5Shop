...
export default class AppreActivity extends Component {
  ...
  componentDidShow() {
    ...
    this.setState({ tabShow: false }, () => {
      getLocation().then((res: any) => {
        that.getTicketInfo(that.$router.params.id, { ypoint: res.latitude || '', xpoint: res.longitude || '' })
      }).catch(err => {
        that.getTicketInfo(that.$router.params.id, { xpoint: '', ypoint: '' })
      })
    })
  }
  ...
  goToPay = (id, e) => {
    ...
    if (this.state.coupon.limit_purchase_quantity && this.state.coupon.user_youhu_log_sum >= this.state.coupon.limit_purchase_quantity) {
      ...
      Taro.navigateTo({
        url: '../../business-pages/coupon-distribution/index?id=' + id + '&couponType=2'
      })
    }
  }
  ...
  render() {
    const { showPoster, posterList, delivery_service_info, tabCurrent, tabContent } = this.state
    ...
    return (
      <View className="appre-activity-detail">
        ...
        {
          tabCurrent == 2 && brief.length ? <View className="img-list-box">
